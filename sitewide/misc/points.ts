...
type saveData = typeof defaultSave;


function milestone(req: number, bonus: number, multAdd: number, incrAdd: number){
 ...
 */
const milestoneBonuses: {req: number; bonus: number; multAdd: number, incrAdd: number}[] = [milestone(60,60,0,0)]


function loadSave() {
    const saved = localStorage.getItem("saveData");
    let save = JSON.parse(saved||stringDefaultSave) as saveData;
...
function pointChecker(save: saveData): saveData {
    console.log(save);
    save.time += 1;
